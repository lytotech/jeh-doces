...
export function AuthScreen() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'forgot') {
        const result = await authRequest<{ message: string }>('/forgot-password', {
          method: 'POST',
      ...
      if (mode === 'resend') {
        const result = await authRequest<{ message: string }>('/resend-verification', {
          method: 'POST',
          body: JSON.stringify({ email }),
        });
        setMessage(result.message);
  ...
  const inputClass =
    'w-full rounded-2xl border border-[#E5DACD] bg-white py-3.5 pl-11 pr-4 text-[#382B20] placeholder:text-[#A99380] focus:border-[#96642F] focus:ring-2 focus:ring-[#B57E44]/15';
  return (
    <main className="min-h-screen bg-[#FAF7F2] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <form
          onSubmit={submit}
          className="bg-white rounded-[2rem] border border-[#E8DECF] shadow-xl shadow-[#7A4B1D]/5 p-7 space-y-4"
        >
          <div>
            <h2 className="font-serif text-2xl font-bold text-[#4A3423]">
              {mode === 'login'
                ? 'Bem-vindo de volta'
                : mode === 'register'
                  ? invite
            ...
            <p className="text-sm text-[#8C7665] mt-1">
              {mode === 'forgot'
                ...
                : mode === 'reset'
                  ...
                  : mode === 'resend'
                    ? 'Abra o link enviado. Ele é válido por 24 horas.'
                    : mode === 'verify'
                      ? 'Validando seu link seguro…'
                      : 'Seus dados ficam separados por empresa.'}
            </p>
          </div>
          {mode === 'register' && (
            <>
              <label className="relative block">
                <User className="absolute left-4 top-4 text-[#A77A4D]" size={18} />
                <input
              ...
              {!invite && (
                <label className="relative block">
                  <Store className="absolute left-4 top-4 text-[#A77A4D]" size={18} />
                  <input
                    className={inputClass}
              ...
              <div className="space-y-2 rounded-2xl bg-[#FAF7F2] p-3 text-xs text-[#6B5747]">
                <label className="flex cursor-pointer items-start gap-2.5">
                  <input
                    type="checkbox"
                    className="mt-0.5 h-4 w-4 accent-[#96642F]"
                ...
                <label className="flex cursor-pointer items-start gap-2.5">
                  ...
                  <span>
                    ...
                    e estou ciente do tratamento dos dados.
                  </span>
                </label>
              </div>
            </>
          ...
          {!['reset', 'verify'].includes(mode) && (
            <label className="relative block">
              <Mail className="absolute left-4 top-4 text-[#A77A4D]" size={18} />
              <input
                type="email"
                className={inputClass}
          ...
          {!['forgot', 'resend', 'verify'].includes(mode) && (
            <label className="relative block">
              <LockKeyhole className="absolute left-4 top-4 text-[#A77A4D]" size={18} />
              <input
              ...
              <button
                type="button"
                className="absolute right-4 top-4 text-[#8C7665]"
          ...
          {mode !== 'verify' && (
            <button
              disabled={busy}
              className="w-full rounded-2xl bg-[#96642F] py-3.5 font-bold text-white hover:bg-[#7A4B1D] disabled:opacity-60"
            >
              {busy
                ? 'Aguarde…'
                : mode === 'login'
          ...
          <div className="text-center text-sm space-y-2">
            {mode === 'login' && (
              <>
                <button
                  type="button"
                  className="block w-full text-[#96642F]"
            ...
            {mode !== 'login' && (
              ...
              >
                {invite ? 'Já tenho uma conta' : 'Voltar para o login'}
              </button>
            )}
          </div>
          <a
            href="/"
            className="block text-center text-xs font-semibold text-[#8C7665] hover:text-[#96642F]"
          >
