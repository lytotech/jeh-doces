  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] pb-28">
      ...
      <div className="max-w-md mx-auto p-4 sm:p-5 space-y-4">
        ...
        <div className="bg-[#F2ECE1] p-4 rounded-2xl border border-[#DFCFC0] shadow-xs space-y-3">
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-xl bg-white/70 border border-[#DFCFC0] text-[#7A4B1D] shrink-0 mt-0.5">
          ...
          <div className="pt-2 border-t border-[#E5DACD] space-y-1.5 text-sm">
            <div className="flex items-center justify-between">
              <span className="text-[#7A6453] font-medium">Cliente</span>
              <span className="text-[#302116] font-semibold">{order.clientName}</span>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-[#7A6453] font-medium">Entrega</span>
            ...
            {order.clientPhone && (
              <div className="flex items-center justify-between">
                <span className="text-[#7A6453] font-medium">Telefone</span>
        ...
        <div className="pt-2">
          <h3 className="text-base font-bold text-[#302116] mb-2.5 px-1">Itens</h3>
          ...
          <div className="space-y-2.5">
            ...
            {order.items.map((item) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-[#FAF3EB] text-[#78542A] flex items-center justify-center shrink-0">
                    <Cookie className="w-4 h-4" />
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-[#302116]">{item.productName}</h4>
                ...
                <span className="text-sm font-bold text-[#302116]">
            ...
            {order.materials.map((mat) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-[#EDE4D8] text-[#78542A] flex items-center justify-center shrink-0">
                    <Package className="w-4 h-4" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-sm font-semibold text-[#302116]">{mat.materialName}</h4>
        ...
        <div className="p-4 bg-[#F2ECE1] rounded-2xl border border-[#DFCFC0] shadow-xs space-y-2.5">
          <div className="flex justify-between items-center text-sm">
            <span className="text-[#6B5747] font-medium">Subtotal</span>
            <span className="text-[#302116] font-semibold">{formatCurrency(order.subtotal)}</span>
          </div>
          ...
          <div className="flex justify-between items-center text-base font-bold pt-1 border-t border-[#E5DACD]">
            ...
            <span className="text-[#302116] text-lg">{formatCurrency(order.totalCharged)}</span>
          </div>

          <div className="flex justify-between items-center text-sm pt-2 border-t border-[#E5DACD]">
            <span className="text-[#7A6453]">Custo estimado</span>
            <span className="text-[#7A6453] font-semibold">{formatCurrency(order.estimatedCost)}</span>
          </div>

          <div className="flex justify-between items-center text-sm">
            <span className="text-[#4A280F] font-bold">Lucro estimado</span>
            <div className="text-right">
              <span className="text-[#96642F] font-bold text-base">
        ...
        <div className="pt-2 space-y-3">
          <div className="flex items-center justify-between">
            ...
            >
              <Plus className="w-3.5 h-3.5 stroke-[3]" /> Registrar pagamento
            </button>
          </div>

          {(!order.payments || order.payments.length === 0) ? (
            <p className="text-xs text-center py-3 text-[#8C7665] bg-white rounded-2xl border border-[#E5DACD] italic">
            ...
            <div className="space-y-2">
              {order.payments.map((p) => (
                <div
                  key={p.id}
                  className="bg-white p-3 rounded-2xl border border-[#E5DACD] flex items-center justify-between shadow-xs"
                >
                  <div className="space-y-0.5">
                    ...
                    {p.notes && <p className="text-[11px] text-[#8A7565] italic">{p.notes}</p>}
                  </div>
              ...
              {/* Status do Pagamento */}
              <div className="p-2.5 bg-amber-50/70 border border-amber-200/80 rounded-xl flex justify-between items-center text-xs">
        ...
        <div className="pt-3">
          <Button
            ...
            className="border-[1.5px] border-[#845025] text-[#845025] bg-white shadow-xs font-semibold"
          >
            <FileText className="w-5 h-5 mr-2 stroke-[2.2]" /> Enviar orçamento
          </Button>
        </div>
      </div>
