...
export const BackupSettingsModal: React.FC<BackupSettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const startRecurring = async (plan: 'monthly' | 'annual') => {
    setBillingLoading(true);
    try { window.location.assign((await api.createRecurringSubscription(plan)).checkoutUrl); } finally { setBillingLoading(false); }
  };
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border border-[#EADDE2] bg-white p-4">
          ...
          {billing && billing.plan === 'basic' ? (
            <div className="mt-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-[#302116]">Plano Básico</p><p className="text-xs text-[#7A6453]">Recursos essenciais liberados.</p></div><div className="flex flex-wrap gap-2"><Button size="sm" disabled={billingLoading} onClick={() => void startPayment('monthly')}>Pix mensal</Button><Button size="sm" disabled={billingLoading} onClick={() => void startRecurring('monthly')}>Automático mensal</Button><Button size="sm" disabled={billingLoading} onClick={() => void startRecurring('annual')}>Automático anual</Button></div></div><p className="mt-2 text-[11px] text-[#8A7565]">A assinatura automática abre o checkout do Mercado Pago e cobra os próximos períodos automaticamente. Pix continua disponível como pagamento manual.</p><p className="mt-3 text-[11px] text-[#8A7565]">Limites: 10 produtos, 20 materiais, 30 ingredientes, 20 encomendas por mês e 1 usuário.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{basicFeatures.map((feature) => <span key={feature} className="flex items-center gap-2 text-xs text-[#5C4533]"><Check className="h-3.5 w-3.5 text-emerald-600" />{feature}</span>)}{completeFeatures.map((feature) => <span key={feature} className="flex items-center gap-2 text-xs text-[#9A8A80]"><Lock className="h-3.5 w-3.5" />{feature} <em className="not-italic text-[10px]">(Completo)</em></span>)}</div></div>
          ) : billing ? (
            <div className="mt-3"><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-[#302116]">Plano Completo <span className="ml-1 text-xs font-normal text-emerald-700">Ativo</span></p><p className="text-xs text-[#7A6453]">Válido até {billing.currentPeriodEnd ? new Date(billing.currentPeriodEnd).toLocaleDateString('pt-BR') : '—'}</p>{billing.status === 'canceled' && <p className="mt-1 text-xs text-amber-700">Renovação cancelada; você continua com acesso até a data acima.</p>}</div>{billing.status !== 'canceled' ? <Button variant="outline" size="sm" disabled={billingLoading} onClick={() => setCancelOpen(true)}>Cancelar renovação</Button> : <Button size="sm" disabled={billingLoading} onClick={() => void startPayment(billing.plan === 'annual' ? 'annual' : 'monthly')}>Renovar plano</Button>}</div><div className="mt-4 grid gap-2 sm:grid-cols-2">{[...basicFeatures, ...completeFeatures].map((feature) => <span key={feature} className="flex items-center gap-2 text-xs text-[#5C4533]"><Check className="h-3.5 w-3.5 text-emerald-600" />{feature}</span>)}</div></div>
