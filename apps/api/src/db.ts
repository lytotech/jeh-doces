...
function mapIngredient(row: IngredientRow): Ingredient {
  return {
    ...
    subIngredients: row.subIngredients.map(({ id, ingredientId, quantity }) => ({
      id,
      ingredientId,
      quantity,
    })),
    priceHistory: row.priceHistory.map(
      ({ id, date, paidPrice, packageQuantity, unitCost, notes }) => ({
        id,
        date: iso(date),
        paidPrice,
        packageQuantity,
...
const ingredientFields = (data: Partial<Ingredient>) => ({
  name: data.name ?? 'Novo Ingrediente',
  isComposite: data.isComposite ?? false,
  unit: data.unit ?? 'g',
  packageQuantity: data.packageQuantity ?? 1000,
...
class Database {
  ...
  async saveIngredient(data: Partial<Ingredient>) {
    const relations = {
      subIngredients: {
        deleteMany: {},
        create: (data.subIngredients ?? []).map(({ id, ingredientId, quantity }) => ({
          ...(id ? { id } : {}),
          ingredientId,
          quantity,
        })),
    ...
    if (
      subIngredientIds.length &&
      (await prisma.ingredient.count({ where: { id: { in: subIngredientIds }, companyId } })) !==
        subIngredientIds.length
    )
    ...
    if (data.id && (await prisma.ingredient.count({ where: { id: data.id, companyId } }))) {
      row = await prisma.ingredient.update({
        where: { id: data.id },
        data: { ...ingredientFields(data), ...relations },
        include: ingredientInclude,
      });
    } else {
      const history = data.priceHistory?.length
        ? data.priceHistory
        : [
            {
  ...
  async deleteIngredient(id: string) {
    return (
      (await prisma.ingredient.deleteMany({ where: { id, companyId: this.companyId() } })).count > 0
    );
  }
  async addPriceHistory(id: string, data: Omit<PriceHistoryRecord, 'id'>) {
    if (!(await prisma.ingredient.count({ where: { id, companyId: this.companyId() } })))
      return null;
    ...
    const row =
      data.id && (await prisma.material.count({ where: { id: data.id, companyId } }))
        ...
        : await prisma.material.create({
            data: { ...(data.id ? { id: data.id } : {}), companyId, ...materialFields(data) },
          });
  ...
  async deleteProduct(id: string) {
    return (
      (await prisma.product.deleteMany({ where: { id, companyId: this.companyId() } })).count > 0
    );
  }
  ...
  async getCustomers(includeArchived = false, search = ''): Promise<Customer[]> {
    ...
    return rows.map((row) => ({
      ...
      notes: row.notes ?? undefined,
      archivedAt: row.archivedAt ? iso(row.archivedAt) : undefined,
      createdAt: iso(row.createdAt),
      updatedAt: iso(row.updatedAt),
    }));
  }
  async getCommitments(): Promise<Commitment[]> {
    return (
      ...
      endsAt: c.endsAt ? iso(c.endsAt) : undefined,
      createdAt: iso(c.createdAt),
      updatedAt: iso(c.updatedAt),
    }));
  }
  async saveCommitment(data: Partial<Commitment>) {
    const companyId = this.companyId();
    const fields = {
    ...
    if (!fields.title) throw new Error('Informe o título do compromisso.');
    const row =
      data.id && (await prisma.commitment.count({ where: { id: data.id, companyId } }))
        ? await prisma.commitment.update({ where: { id: data.id }, data: fields })
        : await prisma.commitment.create({ data: { companyId, ...fields } });
    return {
      id: row.id,
      title: row.title,
      description: row.description ?? undefined,
      startsAt: iso(row.startsAt),
      endsAt: row.endsAt ? iso(row.endsAt) : undefined,
      createdAt: iso(row.createdAt),
      updatedAt: iso(row.updatedAt),
    };
  }
  async deleteCommitment(id: string) {
    return (
      (await prisma.commitment.deleteMany({ where: { id, companyId: this.companyId() } })).count > 0
    );
  }
  async saveCustomer(data: Partial<Customer>) {
    ...
    const row = exists
      ? await prisma.customer.update({ where: { id: data.id }, data: fields })
      : await prisma.customer.create({ data: { companyId, ...fields } });
    return {
      id: row.id,
      name: row.name,
      phone: row.phone ?? undefined,
  ...
  async archiveCustomer(id: string, archived: boolean) {
    ...
    return {
      ...
      archivedAt: updated.archivedAt ? iso(updated.archivedAt) : undefined,
  ...
  async saveOrder(data: Partial<Order>) {
    return prisma.$transaction(async (tx) => {
      ...
      if (
        productIds.length &&
        (await tx.product.count({ where: { id: { in: productIds }, companyId } })) !==
          productIds.length
      )
        throw new Error('Produto relacionado inválido para esta empresa.');
      if (
        materialIds.length &&
      ...
      const row = exists
        ...
        : await tx.order.create({
            data: {
              ...
              ...orderFields(data, orderNumber),
              customerId: customerId ?? null,
              items: { create: relations.items.create },
              materials: { create: relations.materials.create },
              payments: { create: relations.payments.create },
            },
          });
  ...
  async deleteOrder(id: string) {
    return (
      (await prisma.order.deleteMany({ where: { id, companyId: this.companyId() } })).count > 0
    );
  }
  ...
  async addPayment(orderId: string, data: Omit<PaymentRecord, 'id'>) {
    if (!(await prisma.order.count({ where: { id: orderId, companyId: this.companyId() } })))
      return null;
